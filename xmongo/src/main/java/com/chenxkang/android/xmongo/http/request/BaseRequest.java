...
import com.chenxkang.android.xmongo.http.interceptor.HeadersInterceptor;
import com.chenxkang.android.xmongo.http.interceptor.UploadProgressInterceptor;
import com.chenxkang.android.xmongo.http.mode.HttpHeaders;
import com.chenxkang.android.xmongo.util.SSLUtil;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    protected UICallback uploadCallback;//上传进度回调
    ...
    protected void generateLocalConfig() {
        OkHttpClient.Builder newBuilder = MongoHttp.getOkHttpClient().newBuilder();
        ...
        if (isHttpCache) {
            try {
                if (httpGlobalConfig.getHttpCache() == null) {
                    httpGlobalConfig.httpCache(new Cache(httpGlobalConfig.getHttpCacheDirectory(), MongoConfig.CACHE_MAX_SIZE));
                }
        ...
        if (baseUrl != null) {
            ...
            MongoHttp.getRetrofitBuilder().client(newBuilder.build());
            retrofit = MongoHttp.getRetrofitBuilder().build();
        }
    }
    ...
    protected void generateGlobalConfig() {
        httpGlobalConfig = MongoHttp.CONFIG();

        if (httpGlobalConfig.getBaseUrl() == null) {
            httpGlobalConfig.baseUrl(ApiHost.getHost());
        }
        MongoHttp.getRetrofitBuilder().baseUrl(httpGlobalConfig.getBaseUrl());

        if (httpGlobalConfig.getConverterFactory() != null) {
            MongoHttp.getRetrofitBuilder().addConverterFactory(httpGlobalConfig.getConverterFactory());
        }

        if (httpGlobalConfig.getCallAdapterFactory() == null) {
            httpGlobalConfig.callAdapterFactory(RxJava2CallAdapterFactory.create());
        }
        MongoHttp.getRetrofitBuilder().addCallAdapterFactory(httpGlobalConfig.getCallAdapterFactory());

        if (httpGlobalConfig.getCallFactory() != null) {
            MongoHttp.getRetrofitBuilder().callFactory(httpGlobalConfig.getCallFactory());
        }
        ...
        MongoHttp.getOkHttpBuilder().hostnameVerifier(httpGlobalConfig.getHostnameVerifier());

        if (httpGlobalConfig.getSslSocketFactory() == null) {
            httpGlobalConfig.SSLSocketFactory(SSLUtil.getSslSocketFactory(null, null, null));
        }
        MongoHttp.getOkHttpBuilder().sslSocketFactory(httpGlobalConfig.getSslSocketFactory());

        if (httpGlobalConfig.getConnectionPool() == null) {
            httpGlobalConfig.connectionPool(new ConnectionPool(MongoConfig.DEFAULT_MAX_IDLE_CONNECTIONS,
                    MongoConfig.DEFAULT_KEEP_ALIVE_DURATION, TimeUnit.SECONDS));
        }
        MongoHttp.getOkHttpBuilder().connectionPool(httpGlobalConfig.getConnectionPool());

        if (httpGlobalConfig.isCookie() && httpGlobalConfig.getApiCookie() == null) {
            httpGlobalConfig.apiCookie(new ApiCookie(MongoHttp.getContext()));
        }
        if (httpGlobalConfig.isCookie()) {
            MongoHttp.getOkHttpBuilder().cookieJar(httpGlobalConfig.getApiCookie());
        }

        if (httpGlobalConfig.getHttpCacheDirectory() == null) {
            httpGlobalConfig.setHttpCacheDirectory(new File(MongoHttp.getContext().getCacheDir(), MongoConfig.CACHE_HTTP_DIR));
        }
        ...
        if (httpGlobalConfig.getHttpCache() != null) {
            MongoHttp.getOkHttpBuilder().cache(httpGlobalConfig.getHttpCache());
        }
